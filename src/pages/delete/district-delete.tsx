...
const DistrictDelete: React.FunctionComponent = () => {
  ...
  const { district, state } = router.query;

  const del = api.location.delete_district.useMutation({
  ...
  const deleteData = () => {
    confirmed
      ? del.mutate({ state: state as string, district: district as string })
